...
import { GraphicInvokeActionTarget } from './_serverAPI.js'
...
export interface MethodsOnRenderer {
	getManifest: (
		params: EmptyPayload
	) => PromiseLike<{ rendererManifest: RendererInfo & RendererManifest } & VendorExtend>
	// listGraphicInstances: (params: EmptyPayload) => PromiseLike<{ graphicInstances: GraphicInstance[] } & VendorExtend>
	getInfo: (params: EmptyPayload) => PromiseLike<{ rendererInfo: RendererInfo } & VendorExtend>
	getTargetStatus: (
	...
	loadGraphic: (params: { renderTarget: unknown } & RendererLoadGraphicPayload) => PromiseLike<
		{
	...
	) => PromiseLike<{ graphicInstance: GraphicInstanceOnTarget[] } & VendorExtend>
...
export interface MethodsOnServer {
	 ...
	 */
	register: (params: { info: RendererInfo } & VendorExtend) => PromiseLike<{ rendererId: string } & VendorExtend>
	...
	onInfo: (params: { info: RendererInfo } & VendorExtend) => PromiseLike<EmptyPayload>
