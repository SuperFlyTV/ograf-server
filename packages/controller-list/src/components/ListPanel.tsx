			...
			if (actions[e.key]) {
				...
				if (e.key === 'F2' && e.shiftKey) {
					...
					GraphicsListAPI.performAction(selectedItem, 'clear')
						.then(async () => new Promise((r) => setTimeout(r, 100)))
						.then(async () => GraphicsListAPI.performAction(selectedItem, 'load'))
						.then(async () => new Promise((r) => setTimeout(r, 100)))
						.then(async () => GraphicsListAPI.performAction(selectedItem, 'play'))
						.catch(console.error)
					return
				}
