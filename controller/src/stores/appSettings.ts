...
class AppSettings {
  ...
  constructor() {
    ...
    const stateToLoadStr: string | null = window.localStorage.getItem(
      this.LOCALSTORAGE_ID
    );
    try {
      const stateToLoad: StoredState | undefined = stateToLoadStr
        ? JSON.parse(stateToLoadStr)
        : undefined;

      if (stateToLoad?.serverApiUrl)
        this.serverApiUrl = stateToLoad.serverApiUrl;
      if (stateToLoad?.selectedRendererId)
        this.selectedRendererId = stateToLoad.selectedRendererId;
      if (stateToLoad?.queuedGraphics) {
        stateToLoad.queuedGraphics.forEach(([key, value]) =>
          this.queuedGraphics.set(key, value)
        );
      }
    } catch (e) {
  ...
  public getSelectedRendererId(): string | undefined {
    const renderer = serverDataStore.renderersList.find(
      (r) => r.id === appSettingsStore.selectedRendererId
    );

    return renderer?.id;
  }
  public getSelectedRenderer():
