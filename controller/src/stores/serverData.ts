...
class ServerData {
  ...
  public renderersList:
    | OGraf.paths["/renderers"]["get"]["responses"][200]["content"]["application/json"]["renderers"] =
    [];
  public serverInfo:
    | OGraf.paths["/"]["get"]["responses"][200]["content"]["application/json"]
    | undefined;

  public renderersInfo = new ObservableMap<
    string,
    OGraf.components["schemas"]["RendererInfo"]
  >();
  ...
  >();
  public graphicsInstanceMap = new ObservableMap<
    string,
    GraphicsInstanceMapEntry
  >();
      ...
      () => {
        ...
        Promise.resolve()
          .then(async () => {
            ...
            if (this._reloadDataPromise !== undefined)
              await this._reloadDataPromise;
          ...
          .catch((e) => {
            runInAction(() => {
              ...
              this.connectedStatus = errorStr.includes("Failed to fetch")
                ? "Disconnected"
                : "Error in connection";
            });
            console.error(e);
  ...
  private async _reloadData() {
    ...
    {
      const renderTargetMap = new Map<
        string,
        { rendererId: string; renderTarget: unknown }
      >();
      ...
      for (const q of appSettingsStore.queuedGraphics.values()) {
        ...
        if (q.renderTarget) {
          renderTargetMap.set(
            `${q.rendererId}::${JSON.stringify(q.renderTarget)}`,
            {
  ...
  private async doIfEnoughTimeHasPassed<T>(
    ...
    if (duration > MAX_DURATION)
      throw new Error(`Duration too long: ${duration}. Max is ${MAX_DURATION}`);
  ...
  private async _loadServerInfo(asap = false) {
    await this.doIfEnoughTimeHasPassed(
      ...
      async () => {
        ...
        if (r.status === 200)
          runInAction(() => {
            if (!isEqual(this.serverInfo, r.content))
              this.serverInfo = r.content;
          });
      }
    );
  }
  private async _loadGraphicsList(asap = false) {
    await this.doIfEnoughTimeHasPassed(
      ...
      async () => {
        ...
        if (r.status === 200)
          runInAction(() => {
            if (!isEqual(this.graphicsList, r.content.graphics))
              this.graphicsList = r.content.graphics;
          });
      }
    );
  }
  private async _loadRendererList(asap = false) {
    await this.doIfEnoughTimeHasPassed(
      ...
      async () => {
        ...
        if (r.status === 200)
          runInAction(() => {
            if (!isEqual(this.renderersList, r.content.renderers))
              this.renderersList = r.content.renderers;
          });
      }
    );
  }
  ...
  private async _loadGraphic(graphicId: string, asap = false) {
    await this.doIfEnoughTimeHasPassed(
      `_loadGraphic::${graphicId}`,
      asap ? 0 : 10000,
      async () => {
        this.setCurrentOperation(
          `Retrieving graphic info for "${graphicId}"...`
        );
        ...
        if (r.status === 200)
          runInAction(() =>
            mapSetIfNotEqual(this.graphicsInfo, graphicId, r.content)
          );
      }
    );
  }
  private async _loadGraphicsInstances(
    rendererId: string,
          ...
          runInAction(() => {
            if (r.content.renderTarget.graphicInstances) {
              ...
              this.graphicsInstanceMap.forEach((gi, key) => {
                if (
                  gi.rendererId === rendererId &&
                  isEqual(gi.renderTarget, renderTarget) &&
                  !newKeySet.has(key)
                ) {
  ...
  graphicsInstanceMapKey(entry: GraphicsInstanceMapEntry): string {
    return `${entry.rendererId}::${JSON.stringify(entry.renderTarget)}::${
      entry.graphicInstanceId
    }`;
  }
  addToGraphicsInstanceMap = action((entry: GraphicsInstanceMapEntry) => {
    mapSetIfNotEqual(
      this.graphicsInstanceMap,
      this.graphicsInstanceMapKey(entry),
      entry
    );
  });
