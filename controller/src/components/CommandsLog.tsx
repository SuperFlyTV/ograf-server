...
export const CommandsLog = observer(() => {
  const [displayAll, setDisplayAll] = useStoredState<boolean>(
    "commands-log-filter-all",
    true
  );
  ...
  React.useLayoutEffect(() => {
    ...
    const bottomFlex =
      (el.scrollTop === 0 ? 200 : 100) +
      Math.max(100, childHeight) * Math.max(1, addedCount);

    // Scroll to bottom, if already scrolled to bottom:
    if (
      Math.abs(el.scrollTop - (el.scrollHeight - el.clientHeight)) < bottomFlex
    ) {
      // Scroll to bottom
...
const ExpandText = (props: { text: string | undefined }) => {
  ...
  return (
    <span onClick={() => setExpand(!expand)}>
      {expand ? props.text : props.text.slice(0, 100) + "..."}
    </span>
  );
};
