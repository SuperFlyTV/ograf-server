...
import { clone, isEqual, useStoredState } from "../lib/lib.js";
...
export const GraphicsList = observer(() => {
  const entries = Array.from(appSettingsStore.queuedGraphics.entries()).sort(
    (a, b) => {
    ...
    <Box sx={{ m: 1, p: 1 }}>
      ...
      <>
        {appSettingsStore.queuedGraphics.size === 0 ? (
          <>
            <Typography>
              No queued graphics yet, select one in the dropdown below to add
              it.
            </Typography>
          </>
        ) : (
          entries.map(([key, q]) => (
            <QueuedGraphicItem key={key} graphicKey={key} />
          ))
        )}
      </>
    </Box>
...
export const QueuedGraphicItem = observer((props: { graphicKey: string }) => {
  ...
  const graphicsInstances = Array.from(
    serverDataStore.graphicsInstanceMap.values()
  )
    .filter(
      (gi) =>
  ...
  if (graphic.graphic.description)
    descriptions.push(graphic.graphic.description);
  if (graphic.graphic.author)
    descriptions.push(`by ${graphic.graphic.author.name}`);
  ...
  return (
    <Card elevation={1} sx={{ m: 1, p: 1 }}>
      ...
      <CardContent>
        ...
        <Box>
          {!queuedGraphic.renderTarget && (
            <Typography color="error">No render target specified!</Typography>
          )}
        </Box>
            ...
            .renderTargetGraphicInvokeCustomAction(
              ...
              },
              {
                ...
                params: {
                  id: action.id,
                  payload: toJS(
                    queuedGraphic.customActionData[action.id] || {}
                  ),
                  // skipAnimation
